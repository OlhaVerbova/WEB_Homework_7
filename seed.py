...
def fill_data():
    ...
    groups = ["Group 101", "Group 202", "Group 303"]
    ...
    def seed_grades():
        ...
        end_date = datetime.strptime("2023-12-02","%Y-%m-%d")
        ...
        for d in d_range:
            random_id_discepline = choice(discipline_ids)
            ...
            for student_id in random_ids_student:
                grade = Grade(
                    ...
                    discipline_id = random_id_discepline,
                )
                session.add(grade)
            session.commit()
